...
# Sample student data (first 6 students only)
students = [
...
# For loop (6 iterations for the first 6 students)
for student in students:
    state['x'] = student["Student"]
    add_state(14, "")  # For x := 1 To 6 Do Begin
    add_state(15, f"─[ Registering Student {student['Student']} of 6 ]─")
...
# Export to CSV
with open('trace_table_first_6.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(headers)  # Write headers
    writer.writerows(trace_table)  # Write all rows

print(f"Trace table has been exported to 'trace_table_first_6.csv' with {len(trace_table)} lines.")